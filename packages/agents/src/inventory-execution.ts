...
export function createInventoryReorderHandler(repository: InventoryRepository) {
  return async (input: Record<string, unknown>) => {
    ...
    const current = await repository.getLevel("__execution_tenant__", value.variantId, value.locationId);
    if (!current) throw new Error("Inventory level not found");
    return { status: "prepared", variantId: value.variantId, locationId: value.locationId, quantity: value.quantity, currentAvailable: current.available };
  };
}
